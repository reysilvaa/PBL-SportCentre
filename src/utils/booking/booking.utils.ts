...
export const sendErrorResponse = (res: Response, status: number, message: any, details?: any): void => {
...
export const validateBookingTime = async (fieldId: number, bookingDate: Date, startTime: Date, endTime: Date) => {
  ...
  try {
    ...
    switch (eventType) {
      case 'new-booking':
        ...
        if (data.branchId) {
          io.to(`branch-${data.branchId}`).emit('booking:created', data.booking);
        }
