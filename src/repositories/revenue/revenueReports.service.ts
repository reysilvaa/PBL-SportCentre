...
export const generateRevenueReport = async (
  ...
  const totalRevenue = bookingsWithPayments.reduce(
    (sum, booking) => sum + Number(booking.payment?.amount || 0), 0
  );
...
function processBranchRevenue(bookings: any[]): BranchRevenue[] {
  const branchRevenueMap = new Map<number, { branchName: string, revenue: number, bookings: number }>();
  ...
  bookings.forEach(booking => {
    if (booking.field?.branch) {
      ...
      if (branchRevenueMap.has(id)) {
        ...
        branchRevenueMap.set(id, { branchName: name, revenue: amount, bookings: 1 });
      }
    }
  });
  
  return Array.from(branchRevenueMap.entries()).map(([branchId, data]) => ({
    branchId,
...
function processFieldRevenue(bookings: any[]): FieldRevenue[] {
  const fieldRevenueMap = new Map<number, { 
    fieldName: string, 
    branchName: string, 
    revenue: number, 
    bookings: number 
  ...
  bookings.forEach(booking => {
    if (booking.field) {
      ...
      if (fieldRevenueMap.has(id)) {
        ...
        fieldRevenueMap.set(id, { fieldName: name, branchName, revenue: amount, bookings: 1 });
      }
    }
  });
  
  return Array.from(fieldRevenueMap.entries()).map(([fieldId, data]) => ({
    fieldId,
...
async function calculateBranchPerformance(branches: any[], bookings: any[]): Promise<BranchPerformance[]> {
  return branches.map(branch => {
    ...
    const averageBookingValue = branchBookings.length > 0 ? totalRevenue / branchBookings.length : 0;
...
function calculateCustomerRetention(bookings: any[]): CustomerRetention {
  ...
  const returningCustomers = Array.from(userBookingsMap.values()).filter(count => count > 1).length;
...
function calculateMonthComparison(bookings: any[]) {
  ...
  const previousMonthBookings = bookings.filter(b => 
    b.bookingDate >= firstDayPreviousMonth && b.bookingDate < firstDayCurrentMonth
  );
...
function generateForecast(
  ...
  const lastMonth = historicalData.length > 0 
    ...
    : { month: DateUtils.getCurrentYearMonth(), bookings: 100, revenue: 5000 };
