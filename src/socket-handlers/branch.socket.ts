...
export const handleBookingSearch = async (socket: Socket, data: any) => {
  try {
    ...
    if (!socket.data.authenticated || !socket.data.user) {
      socket.emit('auth:required', { message: 'Authentication required for this operation' });
      return;
    }
  ...
  branchNamespace.use(async (socket, next) => {
    try {
      ...
      try {
        ...
        socket.data.user = user;
        socket.data.authenticated = true;
        console.log(`Branch client authenticated: ${socket.id}, User: ${user.id}, Role: ${user.role}`);
        next();
      } catch (authError) {
