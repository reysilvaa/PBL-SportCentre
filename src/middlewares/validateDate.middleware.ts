...
export function validateDateMiddleware(req: Request, res: Response, next: NextFunction): void {
  try {
    ...
    if (bookingDate && isNaN(Date.parse(bookingDate))) {
      res.status(400).json({ error: 'Invalid date format', field: 'bookingDate', message: 'bookingDate must be a valid date string (YYYY-MM-DD)' });
      return;
    }
    ...
    if (!timeRegex.test(startTime) || !timeRegex.test(endTime)) {
      res.status(400).json({ error: 'Invalid time format', message: 'startTime and endTime must be in HH:MM format' });
      return;
    }
